...
if %3()==() ( 
  ...
  set docbook_outputdir=%docbook_document_output%\%1\%2\chunk\
) else (
...
set docbook_use_stylesheet=%docbook_custom_xsl_path%\cdbe_chunk_%2.xsl

:: translate

set XML_CATALOG_FILES=%docbook_catalog_ptah%\catalog.xml
%docbook_libxslt_path%\xsltproc.exe  --timing --xinclude --stringparam use.extensions 0 --output %docbook_outputdir%\index.html   %docbook_use_stylesheet%  %docbook_books_path%\%1\%2\src\%1.xml 
